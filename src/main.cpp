...
#if defined(NDEBUG)
# error "Pop Core cannot be compiled without assertions."
#endif
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
	...
	if(nFees<=0) 
	{
	   return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "insufficient nFees");
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s PCH additional fees, %d delta bytes\n",
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, CClaimTrieCache& trieCache, bool fJustCheck)
{
    ...
    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        ...
        return state.DoS(0, error("ConnectBlock(PCH): couldn't find popnode or superblock payments"),
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Pop Related Inventory Messages
    ...
    */
    ...
    case MSG_POPNODE_PING:
        return mnodeman.mapSeenPopnodePing.count(inv.hash);

    case MSG_DSTX:
        return mapDarksendBroadcastTxes.count(inv.hash);

    case MSG_POPNODE_VERIFY:
        return mnodeman.mapSeenPopnodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_POPNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenPopnodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenPopnodeBroadcast[inv.hash].second;
                ...
                if (!pushed && inv.type == MSG_POPNODE_PING) {
                    if(mnodeman.mapSeenPopnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenPopnodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_POPNODE_VERIFY) {
                    if(mnodeman.mapSeenPopnodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenPopnodeVerification[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CPopnode* pmn = mnodeman.Find(dstx.vin);
            if(pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find popnode %s to verify %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if(!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- Popnode %s is sending too many transactions %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
                // TODO: Not an error? Could it be that someone is relaying old DSTXes
                // we have no idea about (e.g we were offline)? How to handle them?
            }

            if(!dstx.CheckSignature(pmn->pubKeyPopnode)) {
                LogPrint("privatesend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }

            LogPrintf("DSTX -- Got Popnode transaction %s\n", hashTx.ToString());
