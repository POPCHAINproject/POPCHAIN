...
// NOTE: Unlike wallet RPC (which use PCH values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblockrewarddata(const UniValue& params, bool fHelp)
{
	...
	file << strprintf("# block reward created by Pop Core %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Pop Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Pop Core is downloading blocks...");
