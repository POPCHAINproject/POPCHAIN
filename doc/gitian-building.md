...
*Setup instructions for a Gitian build of Pop Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pop
...
- [Building Pop Core](#building-pop-core)
...
```bash
git clone https://github.com/POPCHAINFoundation/POPCHAIN
git clone https://github.com/POPCHAINFoundation/POPCHAIN
```
...
in the Pop Core repository under 'Fetch and build inputs' to install sources which require
...
To build Pop Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Pop Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pop/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/POPCHAINFoundation/POPCHAIN
...
For example:
```bash
URL=https://github.com/POPCHAINFoundation/POPCHAIN
...
./bin/gbuild --commit pop=${COMMIT} --url pop=${URL} ../pop/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pop git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pop/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pop-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pop-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pop-osx-build.assert
```
...
[pop/gitian.sigs](https://github.com/POPCHAINFoundation/POPCHAIN/) repository, or if that's not possible to create a pull
request.
